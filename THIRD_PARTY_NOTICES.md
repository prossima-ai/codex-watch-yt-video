...
This notice inventories only external tools and services used directly by the
current Codex-native `watch` implementation. It does not grant permission to
download, install, redistribute, or use any listed component.
...
**Classification: confirmed. Evidence:** inspection of the pinned implementation
base `0f1f224e818f52853d4d9e8356abf7be14c172a1` and the repository tree on
2026-08-14.

This repository invokes host-provided tools and external provider services; no
third-party code, binary, or license text is bundled. It therefore does not
reproduce full upstream license texts. If a future release distributes a tool,
...
## Provider services and current documentation observations
...
observation, not a live transcription request, account-entitlement check, or
provider-retention probe.

| Service | Implemented adapter target | Official documentation observation | Service/license boundary |
| --- | --- | --- | --- |
...
pinned base and specification [section 8.3](docs/spec/watch-skill.md#83-upload-form-chunking-and-retries).

The current adapters configure a maximum audio chunk payload of 25,165,823 bytes
(`24 * 1024 * 1024 - 1`) before multipart overhead. Current OpenAI documentation
states a 25 MB upload limit, and current Groq documentation states 25 MB for the
free tier and 100 MB for the developer tier. This code has no account-tier limit
discovery, so it cannot truthfully promise that its ceiling is below a 25,000,000
byte provider/account limit. This is an unresolved release gate, not a verified
provider capability. Per the specification, a provider must be disabled for a
release if its required current disclosure or limit cannot be verified and met;
no runtime change is made by this documentation record.

Provider selection, fresh provider-specific audio-upload consent, and separate
provider-network approval remain implementation requirements. Neither the
