...
7. On a visual `partial` outcome, read every selected frame in the listed chronological batches of at most eight using the host's local-image capability. Record one visual observation only after viewing its listed frame, bound to that frame's `chronological_position`. If a frame cannot be read, leave it unobserved; do not simulate inspection or infer its contents from metadata, transcript wording, or its path.
8. For every terminal evidence outcome, follow [the evidence-grounded answer layer](references/answer-layer.md). Declare every evidence stream material to the question; never omit a missing stream to make the answer appear supported. Build claims only from returned metadata/transcript evidence and recorded visual observations, then pass the exact outcome and answer request directly to `python3 -B scripts/answer_watch.py` on standard input. Use the returned `markdown` only according to its `answered`, `withheld`, or `invalid` state. This compiler derives all citations and blocks claims over unavailable or uninspected evidence; never write a parallel answer around it.
9. If 768 px leaves a material small-text or fine-spatial-detail question unresolved, only the typed `FrameInspector` boundary may request a re-extraction of that selected frame at up to 1024 px with a material reason; do not broaden the extraction or infer a reason from keywords.
